...
export async function criarPagamento(_prev: FormState, formData: FormData): Promise<FormState> {
  ...
  const { data: projeto } = await supabase
    .from("projetos")
    .select("data_inicio")
    .eq("id", parsed.data.projeto_id)
    .single<{ data_inicio: string | null }>();
  ...
  if (error) return { erro: "Não foi possível salvar o pagamento." };

  revalidar(parsed.data.projeto_id);
  return { ok: true };
}

/** Marcar como pago registra a data de hoje automaticamente. */
export async function marcarComoPago(id: string, projetoId: string) {
  const supabase = await createClient();
  await supabase.from("pagamentos").update({ data_pagamento: hojeISO() }).eq("id", id);
  revalidar(projetoId);
}
